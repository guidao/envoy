...
AdmissionControlFilterFactory::createFilterFactoryFromProtoTyped(
  ...
  if (!config.rules().empty()) {
    for (const auto& rule : config.rules()) {
      if (rule.has_sr_threshold() && rule.sr_threshold().default_value().value() < 1.0) {
        return absl::InvalidArgumentError("Success rate threshold cannot be less than 1.0% in rule.");
      }
    }
  }
  ...
  if (!config.rules().empty()) {
    ...
    for (const auto& rule_proto : config.rules()) {
      ...
      auto rule_tls = ThreadLocal::TypedSlot<ThreadLocalControllerImpl>::makeUnique(
          context.threadLocal());
      const auto rule_sampling_window = std::chrono::seconds(
          PROTOBUF_GET_MS_OR_DEFAULT(rule_proto, sampling_window,
      ...
      rules.push_back(std::make_shared<AdmissionControlRuleConfig>(
          rule_proto, context.runtime(), context.api().randomGenerator(), dual_info.scope,
          std::move(rule_tls), std::move(rule_evaluator), std::move(rule_filter_headers)));
    }
  }
  ...
  AdmissionControlFilterConfigSharedPtr filter_config =
      std::make_shared<AdmissionControlFilterConfig>(
          ...
          std::move(tls), std::move(response_evaluator), std::move(rules));
